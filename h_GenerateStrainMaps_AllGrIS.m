...
% the entire Greenland Ice sheet. This code relies on and repackages the
...
% generate strain rate maps for 120 model ensemble used to quantify
% uncertainty in fracture predictions across the GrIS. 
%
% Inputs:
...
%
% Outputs:
% Nx3 GeoTIFF file for each grid-aligned strain rates over the entire ice 
% sheet (e_xx, e_yy, and e_xy). 
...
out_dir = "C:\Users\rtc84\Documents\Data\Greenland\Crevasses\IceSlabCrevasseData\GrISStrainRate\";

% Directory where the velocity maps are located
vel_dir = "C:\Users\rtc84\Documents\Data\Greenland\Velocity\MEASURESAverage\";
% Absolute paths to v_x and v_y velocity fields
vx_file = strcat(vel_dir, "greenland_vel_mosaic250_vx_v1.tif");
vy_file = strcat(vel_dir, "greenland_vel_mosaic250_vy_v1.tif");
...
experiment = 1:1:length(thick_grid);   % Array of IDs for experiments
...
ex_file = strcat(vel_dir, "greenland_vel_mosaic250_ex_v1.tif");
ey_file = strcat(vel_dir, "greenland_vel_mosaic250_ey_v1.tif");
...
[ex_in, ~] = readgeoraster(ex_file); % error in x-component of velocity
[ey_in, ~] = readgeoraster(ey_file); % error in y-component of velocity

%% Generate Strain Maps for Each Model
...
for i = err_files
    ...
    for j = 1:length(err_type)
        ...
        vx_new = vx_in + ex_in.*rgf;
        vy_new = vy_in + ey_in.*rgf;
