%% Code for calculating logarithmic strain rates from velocity grids
%{ 
...
function calculateStrain(out_dir, vx_file, vy_file, filt_name, kernel, thick_file,...
    vxNoData, vyNoData, thickNoData, pixel_size, tol, ydir, thick_grid,...
    thick_multiplier, length_scale, maxR, file_code, experiment)

% Define input files and set user-defined parameters

%vx_file = 'D:\Data\Greenland\Velocity\MEASURESAverage\greenland_vel_mosaic250_vx_v1.tif';
%vy_file = 'D:\Data\Greenland\Velocity\MEASURESAverage\greenland_vel_mosaic250_vy_v1.tif';

% Read in velocity geotiffs
[vx_in, vx_info] = geotiffread(vx_file); % x-component of velocity
[vy_in, ~] = geotiffread(vy_file); % y-component of velocity
tiffinfo=geotiffinfo(vx_file); % info from either geotiff

% Smoothing kernel for noisy velocity data - turn off if data is clean
if filt_name == "Box" || filt_name == "Tri"
    vx = conv2(vx_in,kernel,"same");
    vy = conv2(vy_in,kernel,"same");
elseif filt_name == "Gauss"
    vx = imgaussfilt(vx_in,kernel);
    vy = imgaussfilt(vy_in,kernel);
else
    vx = vx_in;
    vy = vy_in;
end
...
% thick_file = 'D:\Data\Greenland\BedMachine\BM_Thickness_250m.tif';
[thick, thick_info] = geotiffread(thick_file);
thickinfo=geotiffinfo(thick_file);
...
% Set grid parameters
% pixel_size = 250; % Pixel size in distance units 
...
% thick_multiplier = 1; % If using an ice thickness grid, the length scale is
...
% length_scale = 250; % If not using an ice thickness grid, define a single 
% % length scale to be used throughout. This is in distance units
% maxR = 12; % Set a maximum value for r, which is length_scale/pixel_size
...
eXXOut = strcat(out_dir,"Exp", num2str(experiment), "_eXX_",file_code,".tif");
eYYOut = strcat(out_dir,"Exp", num2str(experiment), "_eYY_",file_code,".tif");
eXYOut = strcat(out_dir,"Exp", num2str(experiment), "_eXY_",file_code,".tif");
...
geotiffwrite(eXXOut, exGrid, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
geotiffwrite(eYYOut, eyGrid, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
geotiffwrite(eXYOut, exyGrid, vx_info, 'GeoKeyDirectoryTag',tiffinfo.GeoTIFFTags.GeoKeyDirectoryTag);
